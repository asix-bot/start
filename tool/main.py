...
def export_base(
    ...
    for item_id, item_info in item_by_id.items():
        ...
        if exclude_zero_stock and stock_value <= 0:
            ...
            continue
        # Если артикул из 1С уже содержит "/" (формат "число/вариант", например
        # "285/1") - берём его целиком и просто приписываем суфикс. Не пытаемся
        # дополнительно извлечь размер из имени: "/" уже является частью
        # идентификатора товара как он записан в 1С.
        if "/" in raw_article:
